...
{% for db in production_databases %}
    ...
    CREATE OR REPLACE FUNCTION "{{ target.database | trim }}_{{ db | trim }}".{{target.schema}}.crc32("input" string)
    ...
    crc = ( crc >>> 8 ) ^ b_table[( crc ^ input.charCodeAt( i ) ) & 0xFF];
    }
    ...
    GRANT USAGE ON FUNCTION "{{ target.database | trim }}_{{ db | trim }}".{{target.schema}}.crc32(string) TO ROLE PUBLIC; 
